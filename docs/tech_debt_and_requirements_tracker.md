...
| TD-005 | `execution_agent.py` orchestrators remain monolithic: `monitor_portfolio_intraday` (683L), `reconcile_with_ibkr` (589L), `run_market_open_buys` (537L) | Each mixes decision logic with Telegram/IBKR/Supabase I/O, so the risk-critical paths can only be tested through heavy mocking, and no local LLM can hold one in context alongside its callers | Design seams first (inject the notifier, or return events for a thin caller to dispatch), then split. The pure layer was already extracted 2026-09-18 into `exit_rules.py` / `indicators.py` / `market_calendar.py` | Medium | **Deferred — safety-gated.** Tracked in [the Provisional Decision Register](../decisions/provisional_decisions.json) as `orchestrator-split`: becomes due 2026-10-06 and is only actionable when ≤2 positions are open and the youngest is ≥7 days old. Unlike the pure extraction this cannot be byte-identical, so it must not be cut while fresh positions depend on it. See [decisions/2026-09-18_execution-agent-split.md](../decisions/2026-09-18_execution-agent-split.md) |
...
| FU-003 | Day 1 is now covered only by the Early Dollar Stop | Removing the percentage kill-switch from day 1 leaves a wider gap before the Thesis Stop opens on day 2 | — | **Resolved 2026-09-04** — the gap is closed by Prove-It Phase 1, which never expires. See `decisions/2026-09-04_prove-it-stop.md` |
| FU-004 | **Early Dollar Stop sizing** | Shipped as a flat $500, which the full-stack replay showed was net harmful: all-in net −$272 versus +$3,027 at ~$1,500 or with the rule removed. Every loser it caught was already caught a day earlier by the day-0 kill-switch, leaving only winner damage (CPAY −$1,873, DXCM −$1,367). **Resolved 2026-08-20** by making the cap slot-derived — `(equity / EFFECTIVE_POSITION_SLOTS) × 0.06` ≈ $1,500 — rather than removing the rule, since the peak-anchored base trail cannot cover a position that never rose. **Still open:** whether 6% is right. The sample cannot distinguish it from any larger value. | — | **Resolved 2026-09-04** — the rule is retired outright, not resized. It fired zero times in 30 closed trades; Prove-It Phase 1 covers the same band without a day window or dollar arithmetic. See `docs/retired_code.md` |
| FU-005 | Thesis Stop never fired in the 17-trade replay | `THESIS_STOP_ATR_MULT=1.0` over days 2–5 is effectively untested against real trade history | — | **Resolved 2026-09-04** — still zero firings at 30 trades, so the rule was retired rather than retuned. Its `closed_above_entry` latch survives as the Prove-It phase discriminator |
...
| FU-007 | **`EFFECTIVE_POSITION_SLOTS` must become `MAX_POSITIONS`** | The constant existed solely to keep the Early Dollar Stop's slot arithmetic honest while the book was still sized for 4 slots. | — | **Resolved 2026-09-04** — closed without the portfolio reset it was waiting on. The Early Dollar Stop is retired, so the constant has no consumer and was deleted from `execution_agent.py`, `.env.template` and `frontend/src/lib/positionRules.js` |
...
| FU-009 | **Backup job reads on the publishable Supabase key** | `supabase_backup.py` reads every table using `SUPABASE_KEY`, a publishable key that today can read everything because `migrations/20260708_enable_rls_all_tables.sql` defines its policies as `FOR ALL USING (true)`. It works, but broad read access to live trading data on a publishable key is wider than a backup job needs. Consider a dedicated read-only role. | Before any wider sharing of the key, or at the next RLS change | Open |
...
| FU-013 | **AI evaluator is prompted with rules the bot no longer has** | `ai_evaluator.py:263` asks the model for the "Probability the stock hits +25% WITHIN 2-6 WEEKS before -7% stop loss". Neither figure is real: the fixed +25% profit target was removed from the live bot, and the stop is a dynamic 8.25–10% trail that tightens to 1.5% after the +5% lock. Every AI rating is therefore anchored to the wrong risk/reward frame — the model is estimating the odds of a payoff structure the bot does not trade. Rewriting the prompt to the actual exit ladder would change the rating on every candidate, so it needs a measured comparison rather than an in-place edit. Found by the doc-sync grep while removing the same two fabricated numbers from the UI (see `decisions/2026-08-23_exit-detail-panel.md`). | Before the next screening-threshold retune, since entry scores feed the buy gate | **Closed 2026-08-24** — prompt rewritten to the real exit ladder; the `EstDaysTo25%` velocity ladder measured -9.5pp CAGR and was replaced by a volatility-fit band. See `decisions/2026-08-24_ai-evaluator-volatility-fit.md`. |
| FU-014 | **O'Neil's 20-25% profit target is unresolved, not refuted** | `research/oneil_full_system_bt.py` shows the target LOSES on the broad universe at every filter level (-23 to -41pp, P<=1%) but WINS on the screener-passing universe once market timing and entry selectivity are restored (+31.1pp, expectancy +9.94% vs +1.99%). The winning universe is a today-snapshot carrying survivorship bias, which inflates big-run-dependent strategies more than turnover strategies — exactly the artefact that would fake this result. Resolving it requires a POINT-IN-TIME universe rebuilt from `watchlist_history` as of each date, not a today snapshot. Until then the shipped ladder stands. See `decisions/2026-08-24_ai-evaluator-volatility-fit.md`. | Before any change to slot count, exit horizon, or the planned small-cap expansion | Open |
...
| FU-015 | **Three Supabase accessors still swallow read failures** | `get_daily_triggers()`, `get_cash_flows()` and `get_account_balances()` in `backend/database.py` still `return []` on any exception, so an unreachable database is indistinguishable from genuinely empty. `get_positions()` and `get_trade_history()` were converted to raise `DataSourceUnavailable` (→ HTTP 503) on 2026-09-06 because only those two can fabricate a *plausible but false account state*; the remaining three are lower-hazard but carry the same defect. See `decisions/2026-09-06_fail-loudly-on-unreachable-database.md`. | Next time any of these three endpoints is touched | Open |

**Note:** the `FU-010` ID is used twice above (backtester market filter, and the
restore rehearsal). Both entries are live; the collision is an ID bookkeeping
error, not a duplicate item. Next free ID is FU-016.
