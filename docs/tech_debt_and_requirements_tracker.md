...
| FU-008 | Re-derive the winner-side HWM give-back grid on the fixed harness | The +6%/1.5% profit lock was chosen from an ad-hoc daily-bar script, not `research/exit_rule_replay.py`. The chosen row reports *worsened 0* so it is immune to the positive-only aggregation bug, but the rejected +3% row carries a regression and its +$5,311 is likely overstated — the margin between the two is smaller than `decisions/2026-08-20_hwm-profit-lock-first-leg.md` implies. The harness does not yet implement HWM give-back families. | First scheduled review, 2026-09-20 | Open |
