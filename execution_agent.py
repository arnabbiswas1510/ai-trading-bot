...
#
# History: these were widened, then reverted after a replay of the 13 real closed
# trades said the tight ladder was better. That reversion was WRONG — 11 of those
# 12 symbols fail the current fundamental screen, so it was tuning on a
# population being deliberately eliminated (max favourable excursion there was
# only +9.6%; nothing ever reached +20%).
#
# Re-tested on real breakout entries generated by technical_screener's own logic:
#   598 entries in screener-passing names  -> tight +0.11% vs wide +0.18% expectancy
#   2,314 entries in a BROAD unselected universe (includes losers, so not
#   survivorship-biased) -> tight +0.51% vs wide +0.59% expectancy, payoff 1.44 -> 1.58
# The wide ladder wins on both populations, so it is restored.
#
# Note the ratchet in _compute_dynamic_trail_pct() is one-way, so this ladder
# must stay monotonically tightening: you cannot have tight early profit-taking
# AND a wide late trail. This is a deliberate bet on the right tail — in the
# breakout population 46% of entries reach +20% MFE, which the old 2%-at-+20%
# clamp made impossible to capture.
...
TRAIL_PROFIT_TIERS: list[tuple[float, float]] = [
    ...
    ( 0.0, None),    # < 20%      → no change (base STOP_LOSS_PCT applies)
]
...
#     screener-passing names: dCAGR +18.8, 90% CI [+7.1, +33.0], P(better) 100%
#     broad unselected:       dCAGR  -0.7, 90% CI [-15.9, +15.6] — neutral
#     average loss improves in BOTH: -5.60 -> -4.39 and -4.43 -> -3.52
# Exiting via arm_exit() rather than a market sell also won in BOTH universes
# (a market sell scored -7.6 dCAGR, P(better) 21%, in the broad universe).
        ...
        if (THESIS_STOP_ENABLED and not power_held
            ...
            if not _latch:
                ...
                if unrealized_pct <= thesis_threshold_pct:
                    ...
                    # a local trough, and the armed trail beat an immediate sell
                    # in both backtest universes.
                    arm_exit(ib, client, ticker, shares, current_price, reason, now_ny)
