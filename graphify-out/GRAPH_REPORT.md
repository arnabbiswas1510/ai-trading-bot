# Graph Report - ai-trading-bot  (2026-09-09)

## Corpus Check
- 225 files · ~324,314 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3393 nodes · 5324 edges · 305 communities (286 shown, 14 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 68 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f99651c7`
...
- `notifier()` --uses--> `TelegramNotifier`  [INFERRED]
  tests/test_telegram_notifier.py → telegram_notifier.py
...
Cohesion: 0.13
Nodes (10): compute_liquidity_score(), Penalises low-price, low-volume, and small-cap stocks (0-100). Price tier (0-40…, NVDA-like: $750, 42M avg vol, Large -> max score, Mid-tier stock: $35, 800K vol, Mid, SGHC-like: $8, 180K vol, Small -> very low, $15 exact -> price tier = 20, $14.99 -> price tier = 10, $50 exactly -> price tier = 40 (+2 more)
...
Cohesion: 0.12
Nodes (16): Exception, Fires from ai_evaluator.py after all 5-component scores are computed. Shows…, Fires after a successful IBKR market buy order is filled and recorded., Fires when a buy order placement on IBKR fails., Fires when the buy loop is stopped after a failed order attempt. Distinct from…, Sent when the Prove-It Stop arms an exit. Phase 1 fires on a breakout that has…, Concise note when a position's governing exit regime changes. Covers the…, Sent at EOD of Day 3 when a position fails the breakout verdict. A FAIL verdict… (+8 more)

### Community 6 - "make_ib_mock"
Cohesion: 0.06
Nodes (29): get_position_price(), IBKR-first live price for an OPEN position, with FMP fallback. Live trades are…, make_ib_mock(), make_ibkr_fill(), make_ohlcv_data(), make_portfolio_item(), mock_ib(), Factory for an ibkr_fills Supabase row. (+21 more)
...
Cohesion: 0.09
Nodes (29): cancel_ticker_sell_orders(), fetch_ibkr_delayed_price(), place_trailing_stop(), Places a GTC Trailing Stop for an open stock position. Trails stop_loss_pct%…, Cancels all active GTC SELL orders for *ticker* (OCA cleanup before explicit…, Fetch the current price for a contract using IBKR delayed market data (type 3).…, get_ibkr_price(), main() (+21 more)

### Community 13 - "package.json"
Cohesion: 0.08
Nodes (25): dependencies, lucide-react, react, react-dom, recharts, devDependencies, @types/react, @types/react-dom (+17 more)

### Community 14 - "mock_supabase_empty"
Cohesion: 0.29
Nodes (7): mock_supabase_empty(), fixture, Supabase mock with no data in any table., Auto-mock execution_agent.notifier for every test in the suite., Clear the module-level IBKR price-map TTL cache before every test.…, _reset_ibkr_price_map_cache(), _silence_notifier()
...
### Community 20 - "process_exit_requests"
Cohesion: 0.12
Nodes (19): _close_exit_request(), _count_open_positions(), get_supabase_client(), handle_mock_sell(), _is_rth_now(), main_loop(), _position_atr_pct(), process_exit_requests() (+11 more)
...
Cohesion: 0.06
Nodes (32): make_supabase_mock(), Returns a MagicMock Supabase client where each table's queries return realistic…, test_trailing_stop.py — Unit tests for place_trailing_stop() and the self-…, TestExecuteSellCancelsTrailingStopFirst, test_reconcile.py — Tests for reconcile_with_ibkr() four reconcile cases.…, Bug #5 related: PortfolioItem uses .averageCost (NOT .avgCost). The code must…, Case 2: averageCost = 0 → skip insert (prevents ghost $0 positions)., Case 2: no absolute stop_loss price is stored. The `stop_loss` column was a… (+24 more)

### Community 23 - "rotate_positions.py"
Cohesion: 0.13
Nodes (23): config.py — single source of truth for cross-module trading parameters. Every…, _cancel_existing_sells(), _get_portfolio(), main(), _notify(), _pick_from_menu(), _place_sell(), IB (+15 more)
...
Cohesion: 0.16
Nodes (10): compute_rs_score(), Relative Strength score (0-100) vs S&P 500 over the last 12 weeks. Excess…, Stock +20%, SPY +5% -> excess +15% -> 100, Excess exactly 10% -> 100, Excess 5% -> 50 + 5*5 = 75, Same return as SPY -> 50, Excess -5% -> 50 + (-5)*5 = 25, Excess exactly -10% -> max(0, 50-50) = 0 (+2 more)
...
Cohesion: 0.05
Nodes (42): 1. Dynamic trailing stop (IBKR-managed), 1b. Static hard stop — the disconnect-proof floor, 1c. Partial scale-out — book a third of a winner at +4%, 2. The Prove-It Stop — always live, 3. Staleness — day 7+ (feeds Rank & Replace), 4. Rank & Replace — day 7+, Armed Exit — the "smart sale" mechanism, Choosing between the two queue modes (+34 more)
...
### Community 42 - "Client"
Cohesion: 0.10
Nodes (21): enqueue_smart_exit(), extract_fill_commission(), get_oca_managed_tickers(), maybe_notify_sell_state(), persist_fill(), Client, Route an automated sell rule through the Smart OCA Exit queue. Rather than…, Tickers whose exit is currently owned by a Smart OCA request. The automated… (+13 more)
...
### Community 80 - "test_telegram_notifier.py"
Cohesion: 0.15
Nodes (16): notifier(), fixture, patch, Test that a non-200 HTTP response from the Telegram API is explicitly printed…, Test that a network exception (e.g. timeout) is explicitly printed and not…, The IBKR-disconnect alert must be unmistakable: it names the outage and the…, When the position count cannot be fetched (Supabase also down), the alert must…, Returns a configured TelegramNotifier for testing. (+8 more)
...
Cohesion: 0.10
Nodes (26): est_days_to_lock(), scoring.py — Pure scoring functions for the 5-component final_score system. No…, Trading days to reach the +5% profit lock at the average ATR pace. Returns the…, Classify a candidate's ATR against the stop ladder it will actually trade.…, volatility_fit(), telegram_notifier.py — CANSLIM Trading Bot Telegram Notification Module Fires…, _prompt_source(), parametrize (+18 more)
...
Cohesion: 0.08
Nodes (51): Any, basetrail_configs(), _correct_split(), day0_configs(), _env(), eod_configs(), ExitConfig, fetch_5min() (+43 more)
...
### Community 255 - "reconcile_with_ibkr"
Cohesion: 0.13
Nodes (14): _exit_context_suffix(), _fetch_current_rs(), fetch_trade_confirms_for_ticker(), _get_entry_rs(), Build a human- and machine-readable summary of the risk state a position was in…, Attach both commission legs to a trade_history row after it is inserted.…, Full bidirectional reconciliation between IBKR actual positions and Supabase…, Return entry_rs_score for a newly opened position. Prefers the rs_score already… (+6 more)

### Community 256 - "monitor_portfolio_intraday"
Cohesion: 0.12
Nodes (31): build_ibkr_price_map(), _compute_ibkr_price_map(), execute_scale_out(), execute_sell(), get_available_cash(), get_ibkr_account(), get_margin_loan(), get_net_liquidation() (+23 more)
...
Cohesion: 0.12
Nodes (11): compute_final_score(), Weighted blend of 5 components (all 0-100) -> 0-100 final score. Technical 30%…, TestPreBreakoutScoreBoost, tests/test_score_components.py Unit tests for the new 5-component scoring…, NVDA-like scores -> should be around 80, SGHC-like scores -> should be around 40-50, Weighted formula: tech=100, rest=0 -> score = 30, liq=100, rest=0 -> score = 25 (+3 more)
...
### Community 267 - "technical_screener.py"
Cohesion: 0.23
...
### Community 276 - "compute_pre_breakout_quality_score"
Cohesion: 0.15
Nodes (12): check_pre_breakout_coil(), compute_pre_breakout_quality_score(), Detects stocks coiling toward an imminent breakout (VCP / handle setup). ALL…, Quality score 0-100 for a pre-breakout (coiling) trigger. Weights: Pivot…, _coil(), Within 1%, 0 vol ratio, 3/3 closes up -> score == 100., Within 1%, 0.5x vol, 3 closes up -> 40+20+20=80., Within 3%, 0.5x vol, 2 closes up -> 35+20+10=65. (+4 more)
...
### Community 278 - "prove_it_stop_level"
Cohesion: 0.25
...
Cohesion: 0.16
Nodes (21): get_live_price(), place_oca_exit(), Factory for IBKR TRAIL order type. `ib_insync` 0.9.x does not export a…, Places the OCA exit pair on an open position: upper leg LMT SELL @ limit_price…, Fetch current price of a ticker from FMP., TrailingStopOrder(), archive(), cancel_sells() (+13 more)
...
### Community 297 - "execution_agent.py"
Cohesion: 0.05
Nodes (41): arm_exit(), calculate_ema(), calculate_sma(), check_volume_distribution(), compute_momentum_health_score(), compute_rsi(), detect_candlestick_reversals(), fetch_held_position_sentiment() (+33 more)
...
### Community 301 - "TestPreBreakoutCoilPass"
Cohesion: 0.29
...
### Community 302 - "trading_days_between"
Cohesion: 0.18
...
### Community 307 - ".test_hwm_date_not_updated_when_price_falls"
Cohesion: 0.33
...
## Knowledge Gaps
- **969 isolated node(s):** `bar_interval`, `bytes`, `dataset`, `date_max`, `date_min` (+964 more)
