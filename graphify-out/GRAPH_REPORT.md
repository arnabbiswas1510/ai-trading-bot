...
## Summary
- 3416 nodes · 5382 edges · 311 communities (294 shown, 17 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 70 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b97978e1`
...
10. `reconcile_with_ibkr()` - 25 edges
...
Cohesion: 0.06
Nodes (35): Mirrors stdout to a daily rotating log file without touching print() calls., Delete execution_YYYY-MM-DD.log files older than KEEP_DAYS.          Uses the da, TeeLogger, Exception, Fires from ai_evaluator.py after all 5-component scores are computed.         Sh, Fires after a successful IBKR market buy order is filled and recorded., Fires when a buy order placement on IBKR fails., Fires when the buy loop is stopped after a failed order attempt.          Distin (+27 more)
...
### Community 14 - "reconcile_with_ibkr"
Cohesion: 0.10
Nodes (27): arm_exit(), cancel_ticker_sell_orders(), _exit_context_suffix(), get_available_cash(), get_margin_loan(), get_own_cash(), _matches_account(), place_trailing_stop() (+19 more)
...
Cohesion: 0.04
Nodes (51): calculate_ema(), calculate_sma(), check_volume_distribution(), compute_momentum_health_score(), compute_rsi(), detect_candlestick_reversals(), fetch_held_position_sentiment(), fetch_historical_closes_with_dates() (+43 more)
...
Cohesion: 0.07
Nodes (25): test_reconcile.py — Tests for reconcile_with_ibkr() four reconcile cases.  Criti, Bug #5 related: PortfolioItem uses .averageCost (NOT .avgCost).         The code, Case 2: averageCost = 0 → skip insert (prevents ghost $0 positions)., Case 2: no absolute stop_loss price is stored.          The `stop_loss` column w, Case 3: In both, share count differs → update Supabase., IBKR has 150 shares, Supabase says 100 → update Supabase to 150., Case 3: IBKR and Supabase both have 100 shares → no share-count write., The IBKR valuation columns are what let the read-only web container render     t (+17 more)

### Community 23 - "monitor_portfolio_intraday"
Cohesion: 0.08
Nodes (44): _compute_ibkr_price_map(), _count_open_positions(), execute_scale_out(), execute_sell(), _fetch_current_rs(), _fetch_ohlcv(), _get_entry_rs(), get_ibkr_account() (+36 more)
...
Cohesion: 0.09
Nodes (27): make_position(), make_supabase_mock(), make_trigger(), Factory for a portfolio_positions Supabase row.      hwm_rs_score: RS score on t, Factory for a daily_triggers Supabase row.      final_score defaults to 75 (a no, Returns a MagicMock Supabase client where each table's queries return     realis, Regression: ai_evaluator.py silently drops tickers from its batch         ("lost, adjusted_score (post-penalty) remains the primary gate input. (+19 more)
...
Cohesion: 0.07
Nodes (28): 1. Dynamic trailing stop (IBKR-managed), 1b. Static hard stop — the disconnect-proof floor, 1c. Partial scale-out — book a third of a winner at +4%, 3. Staleness — day 7+ (feeds Rank & Replace), 4. Rank & Replace — day 7+, Armed Exit — the "smart sale" mechanism, `buy_price` is reconciled against IBKR's `averageCost`, Choosing between the two queue modes (+20 more)
...
### Community 41 - "Client"
Cohesion: 0.08
Nodes (27): _close_exit_request(), enqueue_smart_exit(), extract_fill_commission(), get_oca_managed_tickers(), maybe_arm_power_hold(), maybe_notify_sell_state(), persist_fill(), Client (+19 more)
...
### Community 298 - "2. The Prove-It Stop — always live"
Cohesion: 0.25
...
### Community 300 - "Smart OCA Managed Exit"
Cohesion: 0.25
...
### Community 304 - "_write_breakout_learning_row"
Cohesion: 0.33
Nodes (6): _build_failed_params_snapshot(), _infer_exit_type(), Classify an exit reason into the breakout_learnings exit_type bucket., Build a failure-parameter snapshot for breakout_learnings.      Preferred source, Persist a single breakout_learnings row. Non-fatal by design., _write_breakout_learning_row()
...
### Community 306 - "_pnl_single_price_map"
Cohesion: 0.25
Nodes (6): _IBKRMark, _MarkContract, _pnl_single_price_map(), Minimal stand-in for an ib_insync Contract inside a synthesized mark., A PortfolioItem-shaped mark synthesized from reqPnLSingle().      Exposes the sa, Return {symbol: _IBKRMark} for the account's STK positions via reqPnLSingle.

### Community 307 - ".test_prior_round_trip_sell_is_excluded_from_the_close"
Cohesion: 0.40
Nodes (4): make_ibkr_fill(), Factory for an ibkr_fills Supabase row., Case 1: the close price must be built only from THIS position's sell fills., TestReconcileSellPriceExcludesPriorRoundTrip
...
### Community 310 - "check_and_run_weekly_watchlist"
Cohesion: 0.40
...
## Knowledge Gaps
- **973 isolated node(s):** `bar_interval`, `bytes`, `dataset`, `date_max`, `date_min` (+968 more)
