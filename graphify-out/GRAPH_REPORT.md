# Graph Report - ai-trading-bot  (2026-08-19)

## Corpus Check
- 167 files · ~236,534 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2556 nodes · 3995 edges · 259 communities (244 shown, 15 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 36 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3f5ba231`
...
6. `make_trigger()` - 30 edges
7. `monitor_portfolio_intraday()` - 28 edges
...
## Communities (259 total, 15 thin omitted)
...
### Community 10 - "_run"
Cohesion: 0.09
Nodes (35): _make_ib(), _make_ohlcv(), _make_pos(), _make_sb(), fixture, tests/test_breakout_verdict.py Tests for the Breakout Verdict (Day 3 EOD),…, Day 3 EOD: price +1.5% AND volume 1.2x avg -> PASS, no sell, no fail notify., Day 3 EOD: price only +0.5% (< 1%) -> FAIL written, notify sent. (+27 more)
...
### Community 17 - "_pos"
Cohesion: 0.13
Nodes (6): _pos(), PCT_FROM_PRICE is momentum-following by construction: re-anchoring to the open…, TestLadderSuspension, TestLimitCap, TestResolveLimit, TestResolveTrail
...
Cohesion: 0.09
Nodes (22): compute_rs_score(), Relative Strength score (0-100) vs S&P 500 over the last 12 weeks. Excess…, check_technical_breakout(), _compute_failure_penalty(), compute_quality_score(), fetch_spy_return_12w(), fetch_with_retry_sync(), get_supabase_client() (+14 more)
...
Cohesion: 0.07
Nodes (30): 1. Dynamic trailing stop (IBKR-managed), 2. Early Loss Kill-switch — days 0–1, 2b. Early Dollar Stop — days 0–5, 3. Thesis Stop — days 2–5, 4. EMA-21 support breach — day 7+, 5. Plateau exit — day 7+, 6. Rank & Replace — day 7+, Armed Exit — the "smart sale" mechanism (+22 more)

### Community 38 - "execution_agent.py"
Cohesion: 0.08
Nodes (54): arm_exit(), cancel_ticker_sell_orders(), check_volume_distribution(), _close_exit_request(), enqueue_smart_exit(), execute_sell(), _fetch_current_rs(), fetch_held_position_sentiment() (+46 more)
...
### Community 224 - "rank_policy_bt.py"
Cohesion: 0.27
...
### Community 229 - "increment_retention"
Cohesion: 0.43
...
Cohesion: 0.29
Nodes (5): _placed(), _queue_sb(), tests/test_oca_managed_exit.py — Smart OCA Managed Exit. The dangerous property…, Supabase mock that keeps exit_requests and portfolio_positions apart., TestQueueBackstops
...
Cohesion: 0.33
Nodes (6): _position_atr_pct(), Resolves the OCA upper leg's limit price from stored *intent*. Requests are…, The ATR percent both OCA legs are sized from, with its provenance. Note this is…, Resolves the OCA lower leg's trailing percent. 'ATR_AUTO' scales the trail to…, resolve_oca_limit_price(), resolve_oca_trail_pct()
...
### Community 253 - "trading_days_between"
Cohesion: 0.40
...
## Knowledge Gaps
- **774 isolated node(s):** `bar_interval`, `bytes`, `dataset`, `date_max`, `date_min` (+769 more)
