...
## Corpus Check
- 212 files · ~304,178 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3201 nodes · 5010 edges · 295 communities (279 shown, 16 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 68 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b96d7775`
...
Cohesion: 0.12
Nodes (11): make_ib_mock(), Creates a mock IB instance whose portfolio() always returns the given symbols., test_trailing_stop.py — Unit tests for place_trailing_stop() and the self-healin, place_trailing_stop() places exactly ONE GTC TRAIL order.     No LimitOrder (pro, TestCancelTickerSellOrders, TestPlaceTrailingStop, Case 4: Cash balance sync from IBKR to Supabase account_balances., Large change in cash → upsert to account_balances called. (+3 more)
...
### Community 20 - "Client"
Cohesion: 0.09
Nodes (23): _close_exit_request(), enqueue_smart_exit(), extract_fill_commission(), get_oca_managed_tickers(), persist_fill(), Client, Route an automated sell rule through the Smart OCA Exit queue.      Rather than, Terminal-state writer for an exit_requests row. (+15 more)
...
### Community 23 - "force_buy.py"
Cohesion: 0.15
Nodes (17): config.py — single source of truth for cross-module trading parameters.  Every v, get_ibkr_price(), main(), _place_buy(), IB, force_buy.py — One-off manual buy trigger (bypasses 9:30 AM time gate).  Run thi, Fetch price via IBKR delayed market data (same as execution_agent buy path)., Place a marketable limit BUY order for a trigger.      Uses IBKR delayed ask + $ (+9 more)
...
Cohesion: 0.10
Nodes (27): make_ibkr_fill(), make_ohlcv_data(), make_position(), make_supabase_mock(), make_trigger(), Factory for a portfolio_positions Supabase row.      hwm_rs_score: RS score on t, Factory for a daily_triggers Supabase row.      final_score defaults to 75 (a no, Factory for an ibkr_fills Supabase row. (+19 more)
...
Cohesion: 0.05
Nodes (39): 1. Dynamic trailing stop (IBKR-managed), 2. The Prove-It Stop — always live, 3. Staleness — day 7+ (feeds Rank & Replace), 4. Rank & Replace — day 7+, Armed Exit — the "smart sale" mechanism, Choosing between the two queue modes, Compact column and ladder, Dashboard: the exit detail panel (+31 more)
...
### Community 83 - "make_portfolio_item"
Cohesion: 0.16
Nodes (12): build_ibkr_price_map(), get_position_price(), Return {symbol: PortfolioItem} for the current account's open positions.      Re, IBKR-first live price for an OPEN position, with FMP fallback.      Live trades, make_portfolio_item(), Mimics an ib_insync PortfolioItem.     IMPORTANT: uses .averageCost (PortfolioIt, tests/test_position_price.py  Unit tests for get_position_price() and build_ibkr, TestBuildIbkrPriceMap (+4 more)
...
Cohesion: 0.09
Nodes (41): Any, _correct_split(), day0_configs(), _env(), ExitConfig, fetch_5min(), fetch_entry_atr_pct(), grid_configs() (+33 more)
...
Cohesion: 0.04
Nodes (50): _build_failed_params_snapshot(), calculate_ema(), calculate_sma(), check_volume_distribution(), compute_momentum_health_score(), compute_rsi(), detect_candlestick_reversals(), _exit_context_suffix() (+42 more)

### Community 256 - "monitor_portfolio_intraday"
Cohesion: 0.09
Nodes (47): arm_exit(), cancel_ticker_sell_orders(), execute_sell(), _fetch_current_rs(), _fetch_ohlcv(), get_available_cash(), _get_entry_rs(), get_ibkr_account() (+39 more)
...
### Community 266 - "flex_query_sync.py"
Cohesion: 0.16
...
### Community 277 - "check_and_run_weekly_watchlist"
Cohesion: 0.40
...
## Knowledge Gaps
- **931 isolated node(s):** `bar_interval`, `bytes`, `dataset`, `date_max`, `date_min` (+926 more)
