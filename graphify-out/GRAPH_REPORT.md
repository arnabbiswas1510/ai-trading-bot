...
## Corpus Check
- 227 files · ~328,196 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3423 nodes · 5374 edges · 304 communities (284 shown, 15 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 68 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4837a8ee`
...
2. `make_ib_mock()` - 85 edges
3. `make_supabase_mock()` - 65 edges
4. `make_position()` - 52 edges
5. `_pos()` - 50 edges
...
## Communities (304 total, 15 thin omitted)
...
Cohesion: 0.17
Nodes (15): arm_exit(), cancel_ticker_sell_orders(), place_trailing_stop(), datetime, Places a GTC Trailing Stop for an open stock position. Trails stop_loss_pct%…, Arms a Day 0-6 loss-cutting exit instead of selling immediately at the trigger…, Cancels all active GTC SELL orders for *ticker* (OCA cleanup before explicit…, get_ibkr_price() (+7 more)
...
Cohesion: 0.06
Nodes (37): calculate_ema(), calculate_sma(), compute_momentum_health_score(), compute_rsi(), detect_candlestick_reversals(), _exit_context_suffix(), fetch_historical_closes_with_dates(), _fetch_market_closes() (+29 more)

### Community 21 - "breakout_bt.py"
Cohesion: 0.15
Nodes (22): daily(), dyn_trail(), find_breakouts(), indicators(), Breakout-population backtest. Addresses a selection-bias problem: the exit…, Daily bars from the committed benchmark dataset — no network, no rate limit.…, Enter at the open the day after the signal; exit per cfg on daily bars., run() (+14 more)

### Community 22 - "test_reconcile.py"
Cohesion: 0.07
Nodes (25): test_reconcile.py — Tests for reconcile_with_ibkr() four reconcile cases.…, Bug #5 related: PortfolioItem uses .averageCost (NOT .avgCost). The code must…, Case 2: averageCost = 0 → skip insert (prevents ghost $0 positions)., Case 2: no absolute stop_loss price is stored. The `stop_loss` column was a…, Case 3: In both, share count differs → update Supabase., IBKR has 150 shares, Supabase says 100 → update Supabase to 150., Case 3: IBKR and Supabase both have 100 shares → no share-count write., The IBKR valuation columns are what let the read-only web container render the… (+17 more)

### Community 23 - "reconcile_with_ibkr"
Cohesion: 0.11
Nodes (34): _compute_ibkr_price_map(), _count_open_positions(), execute_sell(), get_available_cash(), _get_entry_rs(), get_ibkr_account(), get_margin_loan(), get_net_liquidation() (+26 more)
...
Cohesion: 0.09
Nodes (25): compute_rs_score(), est_days_to_lock(), scoring.py — Pure scoring functions for the 5-component final_score system. No…, Trading days to reach the +5% profit lock at the average ATR pace. Returns the…, Relative Strength score (0-100) vs S&P 500 over the last 12 weeks. Excess…, check_technical_breakout(), _compute_failure_penalty(), compute_quality_score() (+17 more)
...
Cohesion: 0.10
Nodes (26): make_position(), make_supabase_mock(), make_trigger(), Factory for a portfolio_positions Supabase row. hwm_rs_score: RS score on the…, Factory for a daily_triggers Supabase row. final_score defaults to 75 (a…, Returns a MagicMock Supabase client where each table's queries return realistic…, Regression: ai_evaluator.py silently drops tickers from its batch ("lost in the…, adjusted_score (post-penalty) remains the primary gate input. (+18 more)
...
### Community 41 - "Client"
Cohesion: 0.07
Nodes (34): _close_exit_request(), enqueue_smart_exit(), execute_scale_out(), extract_fill_commission(), get_oca_managed_tickers(), maybe_notify_sell_state(), persist_fill(), place_oca_exit() (+26 more)
...
### Community 256 - "fetch_ibkr_delayed_price"
Cohesion: 0.12
...
### Community 266 - "monitor_portfolio_intraday"
Cohesion: 0.09
Nodes (26): check_volume_distribution(), _fetch_current_rs(), fetch_held_position_sentiment(), _fetch_ohlcv(), _get_market_regime(), hard_stop_price(), maybe_arm_power_hold(), monitor_portfolio_intraday() (+18 more)

### Community 267 - "_get_week_start"
Cohesion: 0.29
...
### Community 273 - "thesis_reexam_bt.py"
Cohesion: 0.35
Nodes (9): main(), Does an INTRADAY POKE above entry deserve to disarm the Thesis Stop? THE…, load(), simulate(), stats(), main(), period_cagrs(), Re-examination of the Thesis Stop after the look-ahead correction. WHY THIS… (+1 more)
...
### Community 304 - "trading_days_between"
Cohesion: 0.18
...
### Community 307 - ".test_prior_round_trip_sell_is_excluded_from_the_close"
Cohesion: 0.40
Nodes (4): make_ibkr_fill(), Factory for an ibkr_fills Supabase row., Case 1: the close price must be built only from THIS position's sell fills. The…, TestReconcileSellPriceExcludesPriorRoundTrip
...
## Knowledge Gaps
- **980 isolated node(s):** `bar_interval`, `bytes`, `dataset`, `date_max`, `date_min` (+975 more)
