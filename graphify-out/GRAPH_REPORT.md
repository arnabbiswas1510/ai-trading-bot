# Graph Report - ai-trading-bot  (2026-08-15)

## Corpus Check
- 151 files · ~206,071 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2303 nodes · 3604 edges · 234 communities (223 shown, 11 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 28 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `254b8442`
...
## Communities (234 total, 11 thin omitted)
...
Cohesion: 0.07
Nodes (24): Mirrors stdout to a daily rotating log file without touching print() calls., Delete execution_YYYY-MM-DD.log files older than KEEP_DAYS. Uses the date…, TeeLogger, Fires from ai_evaluator.py after all 5-component scores are computed. Shows…, Fires after a successful IBKR market buy order is filled and recorded., Fires when a buy order placement on IBKR fails., Fires when the buy loop is stopped after a failed order attempt. Distinct from…, Sent when the Thesis Stop arms an exit. Fires only for breakouts that have… (+16 more)
...
Cohesion: 0.07
Nodes (47): coverage(), daily(), manifest(), Read-only loader for the committed benchmark price dataset. The dataset lives…, Daily bars ascending by date, or None if the symbol is not in the dataset.…, Sorted union of dates across the given symbols (default: all)., symbols(), _table() (+39 more)
...
Cohesion: 0.14
Nodes (14): AI evaluator, Append-only research tables, Buy gating, Changing parameters safely, Configuration Reference, Credentials, Database schema, Fundamental screener (+6 more)
...
Cohesion: 0.07
Nodes (44): date, arm_exit(), calculate_ema(), calculate_sma(), check_volume_distribution(), compute_momentum_health_score(), compute_rsi(), detect_candlestick_reversals() (+36 more)
...
Cohesion: 0.17
Nodes (15): cancel_ticker_sell_orders(), place_trailing_stop(), Cancels all active GTC SELL orders for *ticker* (OCA cleanup before explicit…, Factory for IBKR TRAIL order type. `ib_insync` 0.9.x does not export a…, Places a GTC Trailing Stop for an open stock position. Trails stop_loss_pct%…, TrailingStopOrder(), get_ibkr_price(), main() (+7 more)
...
### Community 76 - "_run"
Cohesion: 0.16
Nodes (19): _make_ib(), _make_pos(), _make_sb(), tests/test_thesis_stop.py Tests for the Thesis Stop — an ATR-normalised…, Day 4, ATR 3%/day, price -5% -> beyond -3% threshold -> arm exit., The trigger price is usually a local trough. Arming a tight trail beat an…, A fixed percentage is meaningless across names: DXCM moves 4%/day, so a 2% stop…, The failure mode that killed the Intraday Loss Minimiser. (+11 more)
...
Cohesion: 0.13
Nodes (12): _make_df(), 15% below 52w high -> beyond 8% proximity -> None., At or above 52w high -> confirmed breakout territory -> None., Close (77) below SMA-50 (~90) -> below trend -> None., Stock -5% vs SPY +15% -> low RS -> None., Recent 3d avg vol 1.1x 50d avg -> sellers still active -> None., Strictly descending then tiny uptick: must compare vs prior row. Use all-…, 5% below high, vol contracting, 3/3 closes up -> PRE_BREAKOUT. (+4 more)
...
### Community 225 - "run_market_open_buys"
Cohesion: 0.17
Nodes (21): fetch_trade_confirms_for_ticker(), get_available_cash(), get_ibkr_account(), get_margin_loan(), get_own_cash(), get_supabase_client(), handle_mock_sell(), main_loop() (+13 more)
...
- **Why does `fetch_trade_confirms_for_ticker()` connect `flex_query_sync.py` to `run_market_open_buys`, `execution_agent.py`?**
