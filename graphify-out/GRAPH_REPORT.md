# Graph Report - ai-trading-bot  (2026-09-07)

## Corpus Check
- 216 files · ~311,376 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3266 nodes · 5092 edges · 299 communities (280 shown, 14 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 68 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `249ebb6c`
...
6. `monitor_portfolio_intraday()` - 32 edges
...
## Communities (299 total, 14 thin omitted)
...
Cohesion: 0.07
Nodes (31): Exception, Fires from ai_evaluator.py after all 5-component scores are computed. Shows…, Fires after a successful IBKR market buy order is filled and recorded., Fires when a buy order placement on IBKR fails., Fires when the buy loop is stopped after a failed order attempt. Distinct from…, Sent when the Prove-It Stop arms an exit. Phase 1 fires on a breakout that has…, Sent at EOD of Day 3 when a position fails the breakout verdict. A FAIL verdict…, Fires after a successful IBKR market sell order is filled and logged. (+23 more)
...
Cohesion: 0.05
Nodes (51): arm_exit(), calculate_ema(), calculate_sma(), check_volume_distribution(), _close_exit_request(), enqueue_smart_exit(), _exit_context_suffix(), extract_fill_commission() (+43 more)
...
Cohesion: 0.05
Nodes (40): 1. Dynamic trailing stop (IBKR-managed), 1b. Static hard stop — the disconnect-proof floor, 2. The Prove-It Stop — always live, 3. Staleness — day 7+ (feeds Rank & Replace), 4. Rank & Replace — day 7+, Armed Exit — the "smart sale" mechanism, Choosing between the two queue modes, Compact column and ladder (+32 more)
...
### Community 80 - "force_buy.py"
Cohesion: 0.18
Nodes (16): place_oca_exit(), place_protective_stops(), place_trailing_stop(), Places the two-leg protective bracket on an open position, in one OCA group:…, Places the OCA exit pair on an open position: upper leg LMT SELL @ limit_price…, Factory for IBKR TRAIL order type. `ib_insync` 0.9.x does not export a…, Places a GTC Trailing Stop for an open stock position. Trails stop_loss_pct%…, TrailingStopOrder() (+8 more)

### Community 83 - "_get_week_start"
Cohesion: 0.29
...
### Community 98 - "_index_is_bullish"
Cohesion: 0.33
Nodes (6): _fetch_market_closes(), _index_is_bullish(), is_market_bullish(), Sorted (date, close) daily history for `ticker`, oldest first. Returns an empty…, Per-index verdict as ``(above_sma, slope_ok)``, or None if data is unusable.…, CANSLIM 'M' (Market Direction) filter — fail-closed. Bullish requires **every**…
...
### Community 245 - "prove_it_stop_level"
Cohesion: 0.25
...
Cohesion: 0.08
Nodes (45): Any, basetrail_configs(), _correct_split(), day0_configs(), _env(), eod_configs(), ExitConfig, fetch_5min() (+37 more)
...
### Community 255 - "trading_days_between"
Cohesion: 0.18
...
Cohesion: 0.11
Nodes (36): build_ibkr_price_map(), cancel_ticker_sell_orders(), _count_open_positions(), execute_sell(), _fetch_current_rs(), get_available_cash(), _get_entry_rs(), get_ibkr_account() (+28 more)
...
### Community 266 - "_position_atr_pct"
Cohesion: 0.33
...
### Community 269 - "Phase 1 — Extract the TOTP Base32 Secret from IBKR"
Cohesion: 0.29
...
### Community 297 - "TestReconcileCase4"
Cohesion: 0.25
...
### Community 298 - "compute_momentum_health_score"
Cohesion: 0.33
...
### Community 301 - "test_reconcile_detects_short_positions"
Cohesion: 0.50
...
## Knowledge Gaps
- **951 isolated node(s):** `bar_interval`, `bytes`, `dataset`, `date_max`, `date_min` (+946 more)
