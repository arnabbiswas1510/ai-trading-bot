"""Helper script to write test_pre_breakout.py with correct encoding."""
content = '''\
import datetime
import pandas as pd
...
class TestPreBreakoutQualityScore:
    ...
    def test_score_within_5pct(self):
        """Within 5%, 0.8x vol, 2 closes up -> 28+8+10=46."""
        assert compute_pre_breakout_quality_score(-4.0, 0.8, 2, 96.0, 90.0) == 46

    def test_score_within_8pct(self):
        """Within 8%, 0.9x vol, 2 closes up -> 20+4+10=34."""
        assert compute_pre_breakout_quality_score(-7.0, 0.9, 2, 93.0, 90.0) == 34
...
class TestPreBreakoutCoilPass:
    ...
    def test_very_tight_coil_within_1pct(self):
        """Within 1% of pivot -> quality_score >= 80."""
...
class TestPreBreakoutCoilFail:
    ...
    def test_gate_e_only_1_close_up(self):
        """96->95->95.5: only 1 of 3 closes rising -> below min=2 -> None."""
        df = _make_df(recent_closes=[96.0, 95.0, 95.5], recent_vols=[700_000] * 3)
        assert _coil(df, rolling_high=100.0) is None
...
with open(r'tests\test_pre_breakout.py', 'w', encoding='utf-8') as f:
    f.write(content)
